...
function createTextTexture(gl, text, font = "bold 30px monospace", color = "#0000EE", isLink = false) {
    ...
    if (isLink) {
      ...
      context.lineWidth = 2;
      context.beginPath();
      context.moveTo(canvas.width / 2 - textWidth / 2, canvas.height / 2 + 10);
      context.lineTo(canvas.width / 2 + textWidth / 2, canvas.height / 2 + 10);
      context.stroke();
    }
...
class Title {
    constructor({ gl, plane, renderer, text, textColor = "#0000EE", font = "30px sans-serif" }) {
      autoBind(this);
      this.gl = gl;
      this.plane = plane;
    ...
    createMesh() {
      const { texture, width, height } = createTextTexture(
        ...
        this.font,
        this.textColor,
        true // Добавляем подчеркивание для ссылки
      );
  ...
  class DockItem {
    constructor({ gl, plane, renderer, text, textColor = "#0000EE", font = "30px sans-serif", onClick }) {
      autoBind(this);
      this.gl = gl;
      this.plane = plane;
      this.renderer = renderer;
    ...
    createMesh() {
      ...
      this.mesh.element.addEventListener('click', this.onClick);
    }
  }
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position; // Используем исходные координаты вершины
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (450 * this.scale)) / this.screen.height 
